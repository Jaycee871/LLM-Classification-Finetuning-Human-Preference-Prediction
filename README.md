...
## Qwen 1024 submission slug incident

The first authorized Qwen 1,024-token submission push created Kaggle Notebook **version 1** successfully, but Kaggle normalized its title to the actual private kernel slug `packkwanlow/qwen-context1024-experimental-submission` while the GitHub workflow kept polling `packkwanlow/qwen-context1024-submission`. The run therefore received `Permission 'kernels.get' was denied` repeatedly and timed out after ~113 minutes; its competition-submit step was skipped, so **no Qwen competition entry was consumed by that failed orchestration**. The workflow now targets the actual slug, fails immediately on access/slug mismatches, and can resume submission of the already-created version 1 without republishing or retraining it.

**Qwen 1024 submission version-1 repair:** the actual private Kaggle Notebook mounted official `test.csv`, the Qwen base model, and the saved LoRA adapter correctly, then failed before inference because the offline source bundle omitted `src/length_baseline.py`, which `finetune_lora.py` imports. No competition submission occurred. The corrected Notebook bundles that dependency and updates the same private kernel to **version 2**; submission proceeds only if version 2 reaches COMPLETE and contains `submission.csv`.

## Qwen 1,024-token A/B swap ensemble audit

After the 1,024-token single-pass Qwen submission scored **1.35009** on Kaggle while the length-only entry remained **1.06530**, the next inference-only diagnostic tests whether the model's strong A/B order sensitivity can be reduced without retraining. The same saved Qwen LoRA adapter and exact seed-42 1,200-row pilot validation subset are evaluated in both original A/B order and swapped B/A order; swapped probabilities are aligned back to the original classes. We compare the original prediction, swapped-aligned prediction, arithmetic mean, and normalized geometric mean. This is exploratory because the same validation set was already used to choose the 1,024-token context. [Protocol notebook](notebooks/qwen_swap_ensemble_1024.ipynb), [private workflow](.github/workflows/qwen-swap-ensemble.yml). No competition submission is made by this audit.

## Qwen 1,024-token arithmetic A/B ensemble submission

The completed private swap audit on the reused 1,200-row Qwen pilot validation subset found **1.190046** multiclass log loss for an arithmetic mean of original-order and aligned swapped-order probabilities, versus **1.320317** for the original single-pass 1,024-token Qwen. The single-pass Qwen's Kaggle leaderboard score was **1.35009**, while the project's best length-only score remains **1.06530**. An explicitly experimental [swap-ensemble submission Notebook](notebooks/qwen_swap_ensemble_submission.ipynb) now performs the two inference passes on competition test rows, maps swapped classes back to original A/B/tie, averages probabilities, and writes `submission.csv`. [Audit results](docs/QWEN_SWAP_ENSEMBLE_RESULTS.md); [submission workflow](.github/workflows/submit-qwen-swap-ensemble.yml). This uses the same saved adapter with no retraining.
